...
def add_macro_momentum_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Daily momentum (assuming 1-minute data, 1440 periods = 1 day)
    # Adjust periods based on timeframe
    periods_per_day = 1440  # For 1m data
    periods_per_week = periods_per_day * 7

    # Daily momentum
    if len(df) > periods_per_day:
        df['daily_return'] = close.pct_change(periods_per_day)
        df['daily_momentum'] = close / close.shift(periods_per_day) - 1
        df['daily_ma'] = close.rolling(periods_per_day).mean()
        df['daily_ma_ratio'] = close / df['daily_ma']
    else:
        # Fallback for shorter timeframes
        df['daily_return'] = close.pct_change(24)
        df['daily_momentum'] = close / close.shift(24) - 1
        df['daily_ma'] = close.rolling(24).mean()
        df['daily_ma_ratio'] = close / df['daily_ma']

    # Weekly momentum
    weekly_periods = min(periods_per_week, len(df) // 2)
    if weekly_periods > 0:
        ...
        df['weekly_return'] = close.pct_change(168)
        df['weekly_momentum'] = close / close.shift(168) - 1
        df['weekly_ma'] = close.rolling(168).mean()
        df['weekly_ma_ratio'] = close / df['weekly_ma']
...
def load_orderbook_features(config: Config, df: pd.DataFrame) -> pd.DataFrame:
    ...
    if coverage_pct < 10:
        ...
        print(f"  Consider collecting more order book data to cover the OHLCV date range.")
...
def prepare_data(
    ...
    if n < config.SEQUENCE_LENGTH + 10:
        raise ValueError("Not enough samples after deadband filtering")
